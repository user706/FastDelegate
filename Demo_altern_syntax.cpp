...
int main(void)
{
	...
	FastDelegate<void()> noparameterdelegate(&SimpleVoidFunction);
    ...
    typedef FastDelegate<void(int, const char *)> MyDelegate;

	// If you want to have a non-void return value, put it at the end.
    typedef FastDelegate<int(int, const char *)> IntMyDelegate;
